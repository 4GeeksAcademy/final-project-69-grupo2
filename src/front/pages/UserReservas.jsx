...
export const UserReservas = ({ manualCanchaId }) => {
    ...
    // SOLUCIÓN: Usamos navigate para evitar que la página se recargue
    const navegarAReserva = (horaElegida = "") => {
        if (!idFinal) return;
        
        const queryParams = new URLSearchParams({
            complejo: complejo?.name || cancha?.complejo_nombre || "Complejo",
        ...
        // Navegación interna de React (sin recarga de página)
    ...
    if (!cancha && !cargando) return <div className="text-center p-5">Cargando...</div>;

    return (
        <div className="card shadow-sm border-0 p-4 bg-white" style={{ borderRadius: 15 }}>
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h5 className="text-muted small mb-0">{complejo?.name || "Complejo"}</h5>
                ...
                {generarHoras().map(h => {
                    ...
                    return (
                        <button
                            ...
                            type="button" // Se agrega type button por buena práctica
                        ...
                        >
                            {ocupado ? (esMinaReserva ? "MI RESERVA" : "OCUPADO") : h}
                        </button>
                    );
                })}
            </div>
